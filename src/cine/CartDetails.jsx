...
export default function CartDetails({ onCloseCart }) {
  const { cartData, setCartData } = useContext(MovieContext);

  const handleDelete = (event, itemId)=>{
    event.preventDefault();

    const filtered = cartData.filter(d => {
        return d.id !== itemId;
    })
      ...
      <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-[420px] sm:max-w-[600px] lg:max-w-[790px] p-4 max-h-[90vh] overflow-auto">
        <div className="bg-white shadow-md dark:bg-[#12141D] rounded-2xl overflow-hidden p-5 md:p-9">
          ...
          <div className="flex items-center justify-end gap-2">
            <a
              className="rounded-md p-2 md:px-4 inline-flex items-center space-x-2 primary text-[#171923] text-sm"
              href="#"
            >
              <img
                src={Checkout}
